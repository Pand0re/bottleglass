...
*/
	...
	for($i = 0; $i < 1; ++$i) {
		$currentRow = $order;
		$currentRow = str_replace("%%NAME%%", "Verre ра pieds", $currentRow);
		$rows .= $currentRow;
	}
	...
	if (mail($email, $title, $content, $headers)) {
	  mail("contact@bottleglass.ch", "Bottleglass - Commande", "Une commande a щtщ passщe sur le site internet.", $header);
		echo $content;
	}
